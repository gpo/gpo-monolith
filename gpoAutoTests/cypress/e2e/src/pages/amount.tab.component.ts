...
class AmountTabComponent {
    ...
    public getYourContribution() {
        return this.yourContribution.then((e) => {
            return Number(e.text().replace('$ ', ''))
        })
    }

    public chooseFirstDollarOption() {
        return this.additionalContribution1.click().then((e) => {
            return Number(e.text().replace('$ ', ''))
        })
    }
    ...
    public chooseDollarOption(option: number){
        const chooseOption: Record<number, ()=> Cypress.Chainable<number>> = {
            ...
            2: () => {
                return this.additionalContribution2.click().then((e) => {
                    return Number(e.text().replace('$ ', ''))
                })
            },
            3: () =>{
                return this.additionalContribution3.click().then((e) => {
                    return Number(e.text().replace('$ ', ''))
                })
            },
            4: ()=> {
                return this.additionalContribution4.click().then((e) => {
                    return Number(e.text().replace('$ ', ''))
                })
            },
            5: ()=> {
                return this.additionalContribution5.click().then((e) => {
                    return Number(e.text().replace('$ ', ''))
                })
            },
            6: () => {
                return this.additionalContribution6.click().then((e) => {
                    return Number(e.text().replace('$ ', ''))
                })
            }
        }
