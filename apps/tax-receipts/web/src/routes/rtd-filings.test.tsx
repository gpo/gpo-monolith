...
const PREPARE_RESULT = {
  rtdFilingId: 'f1',
  filingName: '2026_RTD_8_030620261000',
  preparedCount: 1,
  artifactId: 'a1',
  sha256: 'a'.repeat(64),
  byteSize: 42,
...
const SEND_RESULT = { rtdFilingId: 'f1', submittedAt: '2026-03-07T12:00:00.000Z' };
  ...
  vi.stubGlobal(
    ...
    vi.fn(async (url: string, init?: RequestInit) => {
      ...
      if (String(url).endsWith('/send') && init?.method === 'POST') {
        return jsonResponse(SEND_RESULT, 201);
      }
      if (String(url).endsWith('/rtd/filings') && init?.method === 'POST') {
        return jsonResponse(PREPARE_RESULT, 201);
      }
...
test('lists a draft row and lets it be selected for preparing', async () => {
  ...
  expect(screen.getByRole('button', { name: /Prepare filing \(1 row\)/ })).toBeInTheDocument();
});
...
test('prepares the selected row with the entered CFO name, reason, and format', async () => {
  draftResponse = ONE_DRAFT_ROW;
  renderPage();
  await screen.findByText('Dana Donor');
  fireEvent.click(screen.getByLabelText('Select Dana Donor'));
  fireEvent.change(screen.getByPlaceholderText('printed on the filing'), {
    target: { value: 'Casey CFO' },
  });
  fireEvent.change(screen.getByPlaceholderText('e.g. period-end RTD filing'), {
    target: { value: 'first filing' },
  });

  const prepareButton = screen.getByRole('button', { name: /Prepare filing \(1 row\)/ });
...
test('a prepared-but-unsent filing offers Download and Send', async () => {
  filingsResponse = UNSENT_FILING;
  renderPage();
...
test('confirming send posts a reason and the filing shows as sent', async () => {
  filingsResponse = UNSENT_FILING;
  renderPage();
  await screen.findByText('2026_RTD_8_030620261000');
  fireEvent.click(screen.getByRole('button', { name: 'Send' }));
  fireEvent.change(screen.getByPlaceholderText('Reason (e.g. emailed to EO 2026-03-06)'), {
    target: { value: 'emailed to EO 2026-03-07' },
  });
  fireEvent.click(screen.getByRole('button', { name: 'Confirm sent' }));

  await waitFor(() => expect(calls.some((c) => c.url.endsWith('/f1/send'))).toBe(true));
  const call = calls.find((c) => c.url.endsWith('/f1/send'))!;
  expect(JSON.parse(call.body!)).toMatchObject({ reason: 'emailed to EO 2026-03-07' });
});

test('an already-sent filing shows the sent date instead of a Send button', async () => {
  filingsResponse = SENT_FILING;
  renderPage();
  await screen.findByText('2026_RTD_8_030620261000');
