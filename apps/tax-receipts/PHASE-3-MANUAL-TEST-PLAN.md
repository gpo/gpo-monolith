...
- [ ] Admin > **Emails** shows "Simulated: nothing is sent" and, without
      `EMAIL_LIVE_SENDING_ALLOWED=true`, the **Send real email** switch stays
      disabled even with a reason typed.
- [ ] Review step: fill the pre-check message and reason, **Send pre-checks**.
      Admin > Emails lists one QUEUED email per donor with an address; **Send
      queued now** turns them SENT with a "simulated" badge, and opening one
      shows a `/donor-precheck/...` link that opens the confirm page.
...
- [ ] Type a letter and reason, **Send 1 email(s)**. Within about 15 seconds
      (or after **Send queued now** in Admin > Emails) the email counts move to
      Sent. The log filters by status, kind, and real or simulated, and
      searches by address, donor, or receipt number.
- [ ] In Admin > Emails, open that email and click **Bounced**. Back in the wizard, the receipt is
...
- [ ] With a real Resend account (optional): set `EMAIL_PROVIDER=resend`,
      ...
      `EMAIL_LIVE_SENDING_ALLOWED=true`, turn on **Send real email** in Admin >
      Emails, and point a Resend webhook at `/webhooks/email` with
      `RESEND_WEBHOOK_SECRET`. Send to
...
- Outside production no email leaves the machine: every send is simulated.
  Admin > Emails is the inbox, and a simulated email's Delivered/Bounced
  buttons stand in for the provider's webhook.
